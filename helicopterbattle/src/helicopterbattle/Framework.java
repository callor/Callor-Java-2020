...
import helicopterbattle.Framework.GameState;
import helicopterbattle.Rank.RankVO;
...
public class Framework extends Canvas {
     ...
     */
    public static enum GameState{STARTING, VISUALIZING, GAME_CONTENT_LOADING, MAIN_MENU, OPTIONS, PLAYING, GAMEOVER, DESTROYED, RANK}
    /**
   ...
   /**
    ...
    private void Initialize()
    {
        ...
        rank = new Rank(frameWidth, frameHeight);
        
        playButton = new Rectangle(frameWidth / 2 - 100, frameHeight / 2 + 100, 190, 50);
    }
    ...
    private void LoadContent()
    {
        try 
        {
            ...
            playImg = ImageIO.read(getClass().getResource("/helicopterbattle/resources/images/PlayMenu.png"));
			rankImg = ImageIO.read(getClass().getResource("/helicopterbattle/resources/images/RankMenu.png"));
        } 
    ...
    public void Draw(Graphics2D g2d)
    {
        switch (gameState)
        {
            ...
            case GAMEOVER:
                ...
                g2d.drawString("Click Play to restart or ESC to exit.", frameWidth/2 - 113, frameHeight/4 + 30);
            ...
            case MAIN_MENU:
                ...
                g2d.drawString("Click Play Button to start the game or ESC to exit.", frameWidth / 2 - 114, frameHeight / 2 + 30);
            break;
//            case OPTIONS:
//                //...
//            break;
    ...
    public void mouseClicked(MouseEvent e) {
    	if(e.getX() > playButton.x && e.getX() < playButton.x + playButton.width
    			&& e.getY() > playButton.y && e.getY() < playButton.y + playButton.height) {
    	...
    	if(e.getX() > rank.rankButton.x && e.getX() < rank.rankButton.x + rank.rankButton.width
    			&& e.getY() > rank.rankButton.y && e.getY() < rank.rankButton.y + rank.rankButton.height) {
    		gameState = GameState.RANK;
    	}
    	
    	if(e.getX() > rank.backButton.x && e.getX() < rank.backButton.x + rank.backButton.width
    			&& e.getY() > rank.backButton.y && e.getY() < rank.backButton.y + rank.backButton.height) {
    				if(playCount==0)
    					gameState = GameState.MAIN_MENU;
    				else 
...
//        }
    ...
    private void drawMenuBackground(Graphics2D g2d){
        ...
        g2d.draw(playButton);
        g2d.drawImage(playImg, playButton.x, playButton.y, playButton.width, playButton.height,null);
        g2d.draw(rank.rankButton);
        g2d.drawImage(rankImg, rank.rankButton.x, rank.rankButton.y, rank.rankButton.width, rank.rankButton.height,null);
    }
}