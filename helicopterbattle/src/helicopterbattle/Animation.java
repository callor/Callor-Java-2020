...
public class Animation {
	...
	// Amount of time between frames in milliseconds. (How many time in milliseconds
	// will be one frame shown before showing next frame?)
	private long frameTime;

	// Time when the frame started showing. (We use this to calculate the time for
	// the next frame.)
	 ...
	 * @param frameHeight
	 *            Height of the frame in animation image "animImage" - height of the
	 *            animation image "animImage".
	 * @param numberOfFrames
	 *            Number of frames in the animation image.
	 * @param frameTime
	 *            Amount of time that each frame will be shown before moving to the
	 *            next one in milliseconds.
	 * @param loop
	 *            Should animation repeat in loop?
	 * @param x
	 ...
	 * @param showDelay
	 *            In milliseconds. How long to wait before starting the animation
	 *            and displaying it?
	 */
	public Animation(BufferedImage animImage, int frameWidth, int frameHeight, int numberOfFrames, long frameTime,
			boolean loop, int x, int y, long showDelay) {
	...
	/**
	 * It checks if it's time to show next frame of the animation. It also checks if
	...
	private void Update() {
		if (timeForNextFrame <= System.currentTimeMillis()) {
			...
			// If the animation is reached the end, we restart it by seting current frame to
			// zero. If the animation isn't loop then we set that animation isn't active.
			...
			// Starting and ending coordinates for cuting the current frame image out of the
			// animation image.
	...
	public void Draw(Graphics2D g2d) {
		...
		if (this.timeOfAnimationCration + this.showDelay <= System.currentTimeMillis())
			g2d.drawImage(animImage, x, y, x + frameWidth, y + frameHeight, startingXOfFrameInImage, 0,
					endingXOfFrameInImage, frameHeight, null);
	}

	public void DrawFrontPropeller(Graphics2D g2d) {
		this.Update();

		if (this.timeOfAnimationCration + this.showDelay <= System.currentTimeMillis())
			g2d.drawImage(animImage, x + (Game.stage - 1), y, (x + frameWidth) - (Game.stage - 1) * Game.difficult,
					(y + frameHeight) - Game.stage, startingXOfFrameInImage, 0, endingXOfFrameInImage, frameHeight,
					null);
	}

	public void DrawRearPropeller(Graphics2D g2d) {
		this.Update();

		if (this.timeOfAnimationCration + this.showDelay <= System.currentTimeMillis())
			g2d.drawImage(animImage, x - (Game.stage - 1) * Game.difficult, y,
					(x + frameWidth) - ((Game.stage - 1) * Game.difficult * 2),
					(y + frameHeight) - ((Game.stage - 1) * Game.difficult), startingXOfFrameInImage, 0,
					endingXOfFrameInImage, frameHeight, null);
	}
}